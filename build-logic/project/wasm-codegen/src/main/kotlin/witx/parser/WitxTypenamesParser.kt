...
internal object WitxTypenamesParser {
    fun parse(): List<WasiTypename> {
        val typenameContent = Thread.currentThread().contextClassLoader.getResourceAsStream("witx/typenames.witx")
        val content = CharStreams.fromStream(typenameContent)
        return parse(content)
    }

    fun parse(
        typenamesFile: File? = null,
    ): List<WasiTypename> {
        return parse(CharStreams.fromFileName(typenamesFile.toString()))
    }

...
public class JvmRenameTest : BaseFileSystemIntegrationTest() {
    ...
    public fun overwriting_open_file_should_fail() {
        ...
        createTestFileSystem().use { fileSystem ->
            ...
            fileSystem.execute(
                Open,
                Open(dstFile.name, tempFolderDirectoryFd, OpenFileFlag.O_RDONLY, 0),
    ...
    public fun overwriting_open_directory_should_fail() {
        ...
        createTestFileSystem().use { fileSystem ->
            fileSystem.execute(
                Open,
                Open(dstDirectory.name, tempFolderDirectoryFd, OpenFileFlag.O_DIRECTORY, 0),
            ).getOrElse { fail("Can not open dst directory") }

...
internal class RandomGet(
    language: WasmLanguage,
    module: WasmModule,
    host: EmbedderHost,
) : BaseWasiWasmNode<RandomGetFunctionHandle>(language, module, RandomGetFunctionHandle(host)) {
    ...
    private fun randomgGet(
        memory: WasmMemory,
        @IntWasmPtr(Byte::class) buf: WasmPtr,
        @SizeType bufLen: Size,
    ): Int = handle.execute(memory.toHostMemory(), buf, bufLen).code
}

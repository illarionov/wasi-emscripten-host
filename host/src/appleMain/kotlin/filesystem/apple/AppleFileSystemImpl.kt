...
internal class AppleFileSystemImpl(
    ...
    private val operations: Map<FileSystemOperation<*, *, *>, FileSystemOperationHandler<*, *, *>> = mapOf(
        ...
        CheckAccess to AppleCheckAccess(fsState.pathResolver),
        Chmod to AppleChmod(fsState.pathResolver),
        ChmodFd to AppleChmodFd(fsState),
        Chown to AppleChown(fsState.pathResolver),
        ...
        Hardlink to AppleHardlink(fsState.pathResolver),
        Mkdir to AppleMkdir(fsState.pathResolver),
        PrestatFd to ApplePrestatFd(fsState),
        Symlink to AppleSymlink(fsState.pathResolver),
        ReadFd to AppleReadFd(fsState),
        ReadDirFd to AppleReadDirFd(fsState),
        ReadLink to AppleReadLink(fsState.pathResolver),
        Rename to AppleRename(fsState.pathResolver),
        SeekFd to AppleSeekFd(fsState),
        SetFdFlags to AppleSetFdFlags(fsState),
        SetTimestamp to AppleSetTimestamp(fsState.pathResolver),
        SetTimestampFd to AppleSetTimestampFd(fsState),
        Stat to AppleStat(fsState.pathResolver),
        StatFd to AppleStatFd(fsState),
        SyncFd to AppleSync(fsState),
        TruncateFd to AppleTruncateFd(fsState),
        UnlinkFile to AppleUnlinkFile(fsState.pathResolver),
        UnlinkDirectory to AppleUnlinkDirectory(fsState.pathResolver),

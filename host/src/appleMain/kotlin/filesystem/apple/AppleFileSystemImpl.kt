...
internal class AppleFileSystemImpl(
    ...
    private val operations: Map<FileSystemOperation<*, *, *>, FileSystemOperationHandler<*, *, *>> = mapOf(
        ...
        CheckAccess to AppleCheckAccess(fsState.fsExecutor),
        Chmod to AppleChmod(fsState.fsExecutor),
        ChmodFd to AppleChmodFd(fsState),
        Chown to AppleChown(fsState.fsExecutor),
        ...
        ReadLink to AppleReadLink(fsState.fsExecutor),
        Rename to AppleRename(fsState.fsExecutor),
        SeekFd to AppleSeekFd(fsState),
        SetFdFlags to AppleSetFdFlags(fsState),
        SetTimestamp to AppleSetTimestamp(fsState.fsExecutor),
        SetTimestampFd to AppleSetTimestampFd(fsState),
        Stat to AppleStat(fsState.fsExecutor),
        StatFd to AppleStatFd(fsState),
        SyncFd to AppleSync(fsState),
        TruncateFd to AppleTruncateFd(fsState),
        UnlinkFile to AppleUnlinkFile(fsState.fsExecutor),
        UnlinkDirectory to AppleUnlinkDirectory(fsState.fsExecutor),

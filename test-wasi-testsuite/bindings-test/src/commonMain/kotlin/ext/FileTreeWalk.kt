...
//  FileTreeWalk.kt with File replaced with Path
...
public class FileTreeWalk internal constructor(
    ...
    override fun iterator(): Iterator<Path> = FileTreeWalkIterator()
    ...
    private abstract class DirectoryState(
        ...
        init {
            val isDirectory = fileSystem.metadataOrNull(rootDir)?.isDirectory ?: false
            check(isDirectory) { "rootDir must be verified to be directory beforehand." }
        }
    }
    ...
    private inner class FileTreeWalkIterator : AbstractIterator<Path>() {
        ...
        init {
            when {
                fileSystem.isDirectory(start) -> state.add(directoryState(start))
                fileSystem.isRegularFile(start) -> state.add(SingleFileState(start))
                else -> done()
            }
        }
        ...
        private tailrec fun gotoNext(): Path? {
            ...
            if (file == null) {
                ...
                if (file == topState.root || !fileSystem.isDirectory(file) || state.size >= maxDepth) {
                    // Proceed to a root directory or a simple file
                    return file
                } else {
                    // Proceed to a sub-directory
                    state.add(directoryState(file))
                    return gotoNext()
                }
            }
        }
        ...
        private inner class BottomUpDirectoryState(rootDir: Path) : DirectoryState(rootDir) {
            ...
            override fun step(): Path? {
                if (!failed && fileList == null) {
                    fileList = try {
                        fileSystem.list(root).toList()
                    } catch (@Suppress("SwallowedException") ex: IOException) {
                        failed = true
                        null
                    }
                }
            ...
            override fun step(): Path? {
                if (!rootVisited) {
                    ...
                    if (fileList == null) {
                        ...
                        fileList = fileSystem.list(root).toList()
        ...
        private inner class SingleFileState(rootFile: Path) : WalkState(rootFile) {
            ...
            init {
                check(fileSystem.isRegularFile(rootFile)) {
                    "rootFile must be verified to be file beforehand."
                }
            }
 ...
 */
public fun FileSystem.walk(
    path: Path,
    direction: FileWalkDirection = FileWalkDirection.TOP_DOWN,
    fileSystem: FileSystem,
): FileTreeWalk = FileTreeWalk(start = path, direction = direction, fileSystem = fileSystem)
 ...
 */
public fun FileSystem.walkTopDown(
    path: Path,
): FileTreeWalk = walk(path, FileWalkDirection.TOP_DOWN, fileSystem = this)
 ...
 */
public fun FileSystem.walkBottomUp(
    path: Path,
): FileTreeWalk = walk(path, FileWalkDirection.BOTTOM_UP, fileSystem = this)
